...
set tumor_area := {k in ROWS, h in 20..30: tumor_values[k, h] > 0};
...
subject to tumor_limit {(k,h) in tumor_area} : sum {i in BEAMS} X[i] * beam_values[i, k, h] >= tumor_min;

# Total dosage at critical location [k,h] should be <= max critical dosage
subject to critical_limit {(k,h) in critical_area} : sum {i in BEAMS} X[i] * beam_values[i, k, h] <= critical_max;